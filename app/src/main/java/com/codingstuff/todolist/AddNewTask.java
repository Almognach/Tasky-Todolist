...
public class AddNewTask extends BottomSheetDialogFragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        mSaveBtn.setOnClickListener(v -> {
            ...
            if (finalIsUpdate) {
                mDB.collection(FirebaseAuth.getInstance().getCurrentUser().getEmail()).document("todolist").collection("list").document(id).update("task", task, "due", dueDate);
                ...
                if (task.isEmpty()) {
                    ...
                    mDB.collection(FirebaseAuth.getInstance().getCurrentUser().getEmail()).document("todolist").collection("list").add(taskMap).addOnCompleteListener(task1 -> {
